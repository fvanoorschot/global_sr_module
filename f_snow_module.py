...
def snow_calculation_mswep(catch_id,work_dir):
    # load p-ep-tas timeseries
    file = glob.glob(f'{work_dir}/output/forcing_timeseries/processed_mswep_gleam_gswp/daily/{catch_id}*.csv')[0]
    ts = pd.read_csv(f'{file}',index_col=0)
    # ts = ts.loc[start_date:end_date] #this should be based on sr - or do this for the full timeseries???
    p = ts.p.values
    tas = ts.tas.values
    ...
    ts['pl'] = pl
    ts['ps'] = ps
    ts.to_csv(f'{work_dir}/output/snow/timeseries_mswep/{catch_id}.csv')
...
def run_function_parallel_snow_mswep(
    catch_list=list,
    work_dir_list=list,
    threads=None
    # threads=100
